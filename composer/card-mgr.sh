...
function createConnectionProfile() {
    ...
    CURRENT_DIR=$PWD
    cd "${DIR}"/fabric/crypto-config/peerOrganizations/market1.stockchainz.com/peers/peer0.market1.stockchainz.com/tls
    CERT=$(awk 'NF {sub(/\r/, ""); printf "%s\\\\n",$0;}' ca.crt)
    cd "$CURRENT_DIR"
    sed -i "s|INSERT_MARKET1_CA_CERT|${CERT}|g" $CONNECTION_PROFILE

    cd "${DIR}"/fabric/crypto-config/peerOrganizations/market2.stockchainz.com/peers/peer0.market2.stockchainz.com/tls
    CERT=$(awk 'NF {sub(/\r/, ""); printf "%s\\\\n",$0;}' ca.crt)
    cd "$CURRENT_DIR"
    sed -i "s|INSERT_MARKET2_CA_CERT|${CERT}|g" $CONNECTION_PROFILE

    cd "${DIR}"/fabric/crypto-config/peerOrganizations/market3.stockchainz.com/peers/peer0.market3.stockchainz.com/tls
    CERT=$(awk 'NF {sub(/\r/, ""); printf "%s\\\\n",$0;}' ca.crt)
    cd "$CURRENT_DIR"
    sed -i "s|INSERT_MARKET3_CA_CERT|${CERT}|g" $CONNECTION_PROFILE
...
function createCard() {
    ...
    CERT="${MSP_DIR}"/signcerts/Admin@market${MARKET}.stockchainz.com-cert.pem
...
function bootstrap() {
    if [ $(ls -1 cards/*.json 2>/dev/null | wc -l) -eq 0 ]; then
        generate
    fi
