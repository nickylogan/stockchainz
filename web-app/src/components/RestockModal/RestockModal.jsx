...
class RestockModal extends Component {
    ...
    handleSubmit() {
        ...
        axios.post(this.config.restServer.url + '/RestockItem', {
            '$class': `${NS}.RestockItem`,
            'item': item,
            'amount': amount,
            'transactionId': '',
            ...
            toast.success('Successfully restocked item');
            if (handleSuccess) handleSuccess();
        }).catch(err => {
            console.
            toast.error(`Failed to restock item #${itemData.itemID}: ` + err);
        ...
        return (
            <Modal show={shown} onHide={close} backdrop={this.state.processing ? 'static' : true}>
                ...
                <Modal.Body>
                    ...
                    <Form.Group as={Row}>
                        <Form.Label column sm="6">Amount to restock: </Form.Label>
